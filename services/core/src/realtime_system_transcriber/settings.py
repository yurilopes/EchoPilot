...
class AppSettings(BaseSettings):
    ...
    port: int = 8765
    log_level: str = "INFO"
    settings_path: Path = Field(default_factory=lambda: runtime_file("settings.json"))
    ui_preferences_path: Path = Field(default_factory=lambda: runtime_file("ui_preferences.json"))
    ...
    @field_validator("settings_path", "ui_preferences_path")
    @classmethod
    def resolve_runtime_path(cls, value: Path) -> Path:
        return value if value.is_absolute() else (CORE_ROOT / value)
