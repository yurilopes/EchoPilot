  ...
  return (
    <div className={`model-row ${isSelected ? "model-row-selected" : ""}`}>
      <div>
        ...
        <div className="muted">downloads: {model.downloads ?? "-"} | installed: {model.installed ? "yes" : "no"}</div>

        <div className="profile-badges">
          <span className="badge">speed: {model.profile.speed}</span>
          <span className="badge">quality: {model.profile.quality}</span>
          <span className="badge">live: {model.profile.live_suitability}</span>
          <span className="badge">size: {model.profile.footprint}</span>
          <span className="badge">state: {model.availability}</span>
        </div>
        ...
        {progress ? (
          <div className="download-progress">
            ...
            {model.download_state === "error" && model.download_error ? <div className="error" style={{ marginTop: 6 }}>Download error: {model.download_error}</div> : null}
          </div>
        ) : null}
      </div>
      ...
      <div className="row">
        {showUse ? (
          <button className="btn" onClick={() => onUse(model.id)} disabled={!canUse}>
            Use
          </button>
        ) : null}
